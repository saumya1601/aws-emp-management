...
const UserFormModal = ({ open, onClose, userData }) => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      setValue('department', userData.department ? userData.department._id : '');
      setSelectedImg(userData.profileImage || null);
    } else {
      setValue('name', '');
      setValue('email', '');
  ...
  return (
    ...
    >
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <Box marginTop={2} display="flex">
            <div className="relative">
              ...
              <Image src={selectedImg || "/default.jpg"} alt="Profile" width={100} height={100} className="rounded-full object-cover border-4" />
            </div>
          </Box>
          ...
          <TextField
            ...
            value={watch('department')} // Watch the department value
            SelectProps={{
              native: true, // Native select instead of Material-UI's MenuItem
            }}
          >
            {loading ? (
      ...
      <ImageCropper open={isCropperOpen} image={cropImage} onClose={() => setCropperOpen(false)} onCrop={handleCrop} />
    </Dialog>
  );
};
